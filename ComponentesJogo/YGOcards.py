...
import ComponentesJogo.YGOplayer as player
...
class Card(ABC):
    ...
    def __init__(self, name, ATK, DEF, type, effect):
        ...
        self.mode = False  # Pode ser em modo de ataque ou defesa, se for um monstro
    ...
    @abstractmethod
    def effect(self, player: player.Player, opponent: player.Player):
        pass
...
class Monster(Card):
    def __init__(self, name, ATK, DEF, type, effect):
        super().__init__(name, ATK, DEF, type, effect)
...
class PoteDaGanancia(Card):
    def __init__(self, name, ATK, DEF, type, effect):
        super().__init__(
            "Pote da Ganância", None, None, CardType.MAGIC, "Compre duas cartas."
        )

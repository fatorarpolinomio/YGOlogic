...
import random
import ComponentesJogo.YGOcards as card
...
standardDeck = [
    ...
    card.Monster("Mago Negro", 2500, 2100, card.CardType.MONSTER, ""),
    card.Monster("Dragão Negro de Olhos Vermelhos", 2400, 2000, card.CardType.MONSTER, ""),
    card.Monster("Dragão Branco de Olhos Azuis", 3000, 2500, card.CardType.MONSTER, ""),
    card.Monster("Dragão Filhote", 1200, 700, card.CardType.MONSTER, ""), # nível 3 (arrumar dps)
    card.Monster("Guardião Celta", 1400, 1200, card.CardType.MONSTER, ""), # nível 4
    card.Monster("Dragão do Brilho", 1900, 1600, card.CardType.MONSTER, ""), # nível 4
    card.Monster("Elfa Mística", 800, 2000, card.CardType.MONSTER, ""), # nível 4
    card.Monster("Kaiser Violento", 1800, 1600, card.CardType.MONSTER, ""), # nível 5
    card.Monster("Caveira Invocada", 2500, 1200, card.CardType.MONSTER, ""), #nível 6
...
class Player():
    ...
    def __init__(self, name, life):
        ...
        self.monstersCount = 0; # Contador para a quantidade de monstros que o jogador controla
        self.spellsAndTrapsInField = [];
        self.spellsAndTrapsCount = 0; # Contador para a quantidade de Spells e Traps no campo
    ...
    def summonMonster(self, monster: card.Card):
        ...
        print(f"Deseja invocar {monster.name} em qual modo?");
        print("1)Ataque");
        print("2)Defesa");

        mode = int(input("Digite sua escolha: "));

        while mode != 1 and mode != 2:
            mode = int(input("Escolha inválida, tente novamente: "));

        if mode == 1:
            monster.mode = True;
            self.monstersInField.append(monster);
            print(f"Você invocou {monster.name} em modo de ataque!");

        else:
            monster.mode = False;
            self.monstersInField.append(monster);
            print(f"Você invocou {monster.name} em modo de defesa!");
